      ...
      <div className={s.container}>
        <div className={`${s.leftSide} ${form.type !== '' ? s.slideIn : s.slideOut}`}>
          ...
          {form.type === '' ?
              ...
              <BouquetDetails itemId={form.id} title={form.title} action={form.action} onClose={() => setForm({ type: '', id: undefined })}/>
          )}
        </div>
            ...
            <Tabs defaultActiveKey="types" size='large' animated>
              ...
              <TabPane tab="Букеты" key="bouquets">
                <Button
                    ...
                    style={{ marginBottom: 16 }}
                />
                <BouquetsTable dataSource={dataSourceBouquets} setEditBouquetId={(v) => setForm({type: 'boquet', id: v.id, action: 'update', title: 'Обновление букеты ' + v.name})}/>
              </TabPane>
            </Tabs>
        </div>
        <ModalItemCategory />
